    ...
    <section className="w-full py-12 bg-white">
      <div className="container">
        <div className="text-center mb-8">
          <h2 className="text-2xl md:text-3xl font-bold mb-2">Travel destinations</h2>
          <p className="text-base text-gray-600">Trips for 18–35s across 6 continents.</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {destinations.map((destination, index) => (
            <Link 
              key={index} 
              to={destination.link}
              className="relative h-[200px] overflow-hidden rounded-lg group hover:scale-[1.02] transition-transform duration-300"
            >
              <div
              ...
              <div className="absolute bottom-0 left-0 w-full p-4">
                <h3 className="text-white font-medium text-center">{destination.name}</h3>
              </div>
            </Link>
          ))}
        </div>
