            ...
            <div className="contiki-card">
              <Accordion type="single" collapsible className="w-full">
                {generalFAQs.map((faq, index) => (
                  ...
                  >
                    <AccordionTrigger className="flex items-center justify-between w-full hover:no-underline focus:outline-none focus:ring-0 text-left group -m-6 p-6">
                      <h3 className="text-base font-bold text-black text-left pr-4 leading-tight flex-1">
                        {faq.question}
                      </h3>
                      <div className="flex-shrink-0 w-8 h-8 bg-[#CCFF00] rounded-full flex items-center justify-center transform transition-transform duration-200 group-[&[data-state=open]]:rotate-45">
                        <Plus className="w-5 h-5 text-black group-[&[data-state=open]]:hidden" />
                        <Minus className="w-5 h-5 text-black group-[&[data-state=closed]]:hidden" />
                      </div>
                    </AccordionTrigger>
                ...
                {tripFAQs.map((faq, index) => (
                  ...
                  >
                    <AccordionTrigger className="flex items-center justify-between w-full hover:no-underline focus:outline-none focus:ring-0 text-left group -m-6 p-6">
                      ...
                      <div className="flex-shrink-0 w-8 h-8 bg-[#CCFF00] rounded-full flex items-center justify-center ml-auto transform transition-transform duration-200 group-[&[data-state=open]]:rotate-45">
                        <Plus className="w-5 h-5 text-black group-[&[data-state=open]]:hidden" />
                        <Minus className="w-5 h-5 text-black group-[&[data-state=closed]]:hidden" />
                      </div>
                    </AccordionTrigger>
